...
14. **Foreground Runtime & Media Notifications**: Tools that keep working in the background (playback, sensors, transfers) must use the shared services — never create their own Android service or notification. Android: `ForegroundRuntimeService` (`lib/services/foreground_runtime_service.dart`) lease model; pass a `MediaNotificationData` for audio tools to get a MediaStyle notification with seek bar. Desktop media keys: `MediaControlsService.instance` (`lib/services/media_controls_service.dart`, Windows SMTC / Linux MPRIS) with `buttonEvents`. Non-media background work: `BackgroundWorkLease` (`lib/services/background_work_lease.dart`). Release all leases/controls via `onDispose`. See AGENTS.detail.md §5.
