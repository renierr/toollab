...
- Tools that require data syncing can implement `SyncDelegate`. Register it in `AppState` (constructor in `lib/providers/app_state.dart`) via `registerSyncDelegate()` — this makes the global Settings "Sync Now" button cover that tool. For database storage tracking, protocol flows, and serialization requirements, see the detailed technical specification in [AGENTS.detail.md](AGENTS.detail.md#1-bidirectional-cloud-synchronization).
